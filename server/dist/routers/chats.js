...
// chat create 
router.post("/chat-privacy", async (req, res) => {
    const userId = req.user;
    const recieverName = req.body.username;
    if (!recieverName)
        return res.status(401).json({ message: "Doesn't recevies the reciver name ", status: false });
    try {
...
router.get("/group-messages/:groupId", async (req, res) => {
    ...
    if (!groupId)
        return res.status(400).json({ message: "groupId is required", status: false });
    try {
        const message = await prisma.message.findMany({
            where: {
                groupId: groupId
